FROM condaforge/mambaforge:latest
LABEL io.github.snakemake.containerized="true"
LABEL io.github.snakemake.conda_env_hash="a60e9e3d63e73d88bca7d5c41e60493dc08baf9ed451c4c4125322b1674a2f66"

# Step 1: Retrieve conda environments

# Conda environment:
#   source: envs/xengsort.yaml
#   prefix: /conda-envs/467531a44fae6810670aaa38b00b0866
#   name: xengsort
#   channels:
#     - r
#     - conda-forge
#     - bioconda
#   dependencies:
#     - argcomplete=3.5.1=pyhd8ed1ab_0
#     - attrs=24.2.0=pyh71513ae_0
#     - brotli-python=1.1.0=py312h5861a67_2
#     - bzip2=1.0.8=hfdf4475_7
#     - ca-certificates=2024.8.30=h8857fd0_0
#     - certifi=2024.8.30=pyhd8ed1ab_0
#     - cffi=1.17.1=py312hf857d28_0
#     - charset-normalizer=3.4.0=pyhd8ed1ab_0
#     - colorama=0.4.6=pyhd8ed1ab_0
#     - dataclasses=0.8=pyhc8e2a94_3
#     - docstring_parser=0.16=pyhd8ed1ab_0
#     - exceptiongroup=1.2.2=pyhd8ed1ab_0
#     - fsspec=2024.9.0=pyhff2d567_0
#     - h2=4.1.0=pyhd8ed1ab_0
#     - hpack=4.0.0=pyh9f0ad1d_0
#     - hyperframe=6.0.1=pyhd8ed1ab_0
#     - idna=3.10=pyhd8ed1ab_0
#     - importlib_resources=6.4.5=pyhd8ed1ab_0
#     - iniconfig=2.0.0=pyhd8ed1ab_0
#     - jsonargparse=4.33.2=pyhd8ed1ab_0
#     - jsonnet=0.20.0=py312heafc425_1
#     - jsonschema=4.23.0=pyhd8ed1ab_0
#     - jsonschema-specifications=2024.10.1=pyhd8ed1ab_0
#     - libblas=3.9.0=22_osx64_openblas
#     - libcblas=3.9.0=22_osx64_openblas
#     - libcxx=19.1.2=hf95d169_0
#     - libexpat=2.6.3=hac325c4_0
#     - libffi=3.4.2=h0d85af4_5
#     - libgfortran=5.0.0=13_2_0_h97931a8_3
#     - libgfortran5=13.2.0=h2873a65_3
#     - liblapack=3.9.0=22_osx64_openblas
#     - libllvm14=14.0.6=hc8e404f_4
#     - libopenblas=0.3.27=openmp_h8869122_1
#     - libsqlite=3.46.1=h4b8f8c9_0
#     - libzlib=1.3.1=hd23fc13_2
#     - llvm-openmp=19.1.1=hf78d878_1
#     - llvmlite=0.43.0=py312hcc8fd36_1
#     - ncurses=6.5=hf036a51_1
#     - numba=0.60.0=py312hc3b515d_0
#     - numpy=2.0.2=py312he4d506f_0
#     - openssl=3.3.2=hd23fc13_0
#     - packaging=24.1=pyhd8ed1ab_0
#     - pigz=2.8=h92b6c6a_0
#     - pip=24.2=pyh8b19718_1
#     - pkgutil-resolve-name=1.3.10=pyhd8ed1ab_1
#     - pluggy=1.5.0=pyhd8ed1ab_0
#     - pycparser=2.22=pyhd8ed1ab_0
#     - pysocks=1.7.1=pyha2e5f31_6
#     - pytest=8.3.3=pyhd8ed1ab_0
#     - python=3.12.7=h8f8b54e_0_cpython
#     - python_abi=3.12=5_cp312
#     - pyyaml=6.0.2=py312hb553811_1
#     - readline=8.2=h9e318b2_1
#     - referencing=0.35.1=pyhd8ed1ab_0
#     - requests=2.32.3=pyhd8ed1ab_0
#     - rpds-py=0.20.0=py312h669792a_1
#     - setuptools=75.1.0=pyhd8ed1ab_0
#     - tk=8.6.13=h1abcd95_1
#     - tomli=2.0.2=pyhd8ed1ab_0
#     - tzdata=2024b=hc8b5060_0
#     - urllib3=2.2.3=pyhd8ed1ab_0
#     - validators=0.34.0=pyhd8ed1ab_0
#     - wheel=0.44.0=pyhd8ed1ab_0
#     - xz=5.2.6=h775f41a_0
#     - yaml=0.2.5=h0d85af4_2
#     - zipp=3.20.2=pyhd8ed1ab_0
#     - zstandard=0.23.0=py312h7122b0e_1
#     - zstd=1.5.6=h915ae27_0
#     - pip:
#         - xengsort==2.0.8
#   prefix: /Users/preskaa/miniforge_x86_64/envs/xengsort
RUN mkdir -p /conda-envs/467531a44fae6810670aaa38b00b0866
COPY envs/xengsort.yaml /conda-envs/467531a44fae6810670aaa38b00b0866/environment.yaml

# Step 2: Generate conda environments

RUN mamba env create --prefix /conda-envs/467531a44fae6810670aaa38b00b0866 --file /conda-envs/467531a44fae6810670aaa38b00b0866/environment.yaml && \
    mamba clean --all -y
